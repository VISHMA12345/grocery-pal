...
import { getActiveList, cancelBasketAPI } from '@/api/basketApi';
...
const BasketPage = () => {
  ...
  useEffect(() => {
    setLoading(true);
    getActiveList(activeTab).then((res) => {
      setBaskets(res.data);
      setLoading(false);
    }).catch(() => {
  ...
  const pending = baskets;
  const completed = baskets;
  const cancelled = baskets;

  const cancelBasket = async (_id: string) => {
    try {
      await cancelBasketAPI(_id);
      updateBasketStatus(_id, 'cancelled');
      setBaskets(baskets.filter(b => b._id !== _id));
      toast.success('Basket cancelled');
    } catch (error) {
      toast.error('Failed to cancel basket');
    }
  };
  ...
  const renderBasket = (basket: typeof baskets[0], showActions = false) => (
    <div key={basket._id} className="grocery-card space-y-3">
      <div>
        ...
        <span className={`inline-block mt-1 text-xs font-semibold px-2 py-0.5 rounded-full ${basket.status === 'pending' || basket.status === 'started' ? 'bg-primary/15 text-primary' :
    ...
    <MainLayout title="Baskets">
      <div className="animate-fade-in space-y-4">
        {loading ? (
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} defaultValue="pending" className="w-full">
            <TabsList className="w-full bg-secondary/50 p-1 rounded-xl">
              <TabsTrigger value="pending" className="flex-1 rounded-lg py-2 transition-all">Pending</TabsTrigger>
              <TabsTrigger value="completed" className="flex-1 rounded-lg py-2 transition-all">Done</TabsTrigger>
              <TabsTrigger value="cancelled" className="flex-1 rounded-lg py-2 transition-all">Cancelled</TabsTrigger>
            </TabsList>
            <TabsContent value="pending" className="space-y-4 mt-6">
              ...
              ) : pending.map((b) => renderBasket(b, true))}
            </TabsContent>
